...
class EASY_ComputerPlayer(Player):
	...
	def make_bet(self, current_bet):
		...
		if current_bet is None:
			...
			if current_bet.value == 1 or self.game.is_palifico_round():
				# There should be twice as many of any value than 1
				limit = ceil(total_dice_estimate/4.0) + random.randrange(0, ceil(total_dice_estimate/4.0))
			else:
				limit = ceil(total_dice_estimate/4.0) * 2 + random.randrange(0, ceil(total_dice_estimate/4.0))
			if current_bet.quantity >= limit:
				return DUDO
			else:
				bet = None
				while bet is None:
					if self.game.is_palifico_round() and self.palifico_round == -1:
						...
						# the value cannot be changed.
						value = current_bet.value
						quantity = current_bet.quantity + random.randrange(0, 3)
					else:
						value = random.choice(self.dice).value
						if value == 1:
							if current_bet.value > 1:
							...
							if current_bet.value == 1:
								quantity = current_bet.quantity * 2 + random.randrange(0, 3)
							else:
								quantity = current_bet.quantity + random.randrange(0, 3)
					try:
						bet = create_bet(quantity, value, current_bet, self, self.game)
