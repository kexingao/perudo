...
class Perudo(object):
	...
	def __init__(self, name, player_number, dice_number, dqn_player:DQNPlayer):
		...
		if len(self.players) == 1:
			...
			if self.players[0] == self.dqn_player:
				self.dqn_player.win = 1
				fn = '{}win_{}_{}_'.format(self.dqn_player.name, player_number+1, dice_number)
				k = 0
				if os.path.exists('./checkpoint/' + fn + str(k)):
					k+=1
				self.dqn_player.save_model(fn=fn + str(k))
		else:
	...
	def run_round(self):
		...
		while not round_over:
			...
			if next_bet == DUDO:
				...
				if self.get_previous_player(current_player) == self.dqn_player and ai_has_play:
					self.dqn_player.store_transition(current_bet, 0)
	...
	def run_dudo(self, player, bet):
		...
		if dice_count >= bet.quantity:
			...
			if self.get_previous_player(player) == self.dqn_player:
				self.dqn_player.store_transition(False, 1)
			...
			if player == self.dqn_player:
				# self.dqn_player.store_transition(False, 0)
				self.dqn_player.store_transition(False, 1)
	...
	def remove_die(self, player):
		...
		if len(player.dice) == 0:
			if player == self.dqn_player:
				self.dqn_player.store_transition(False, -1, 1)
...
def main():
	AI = DQNPlayer("DQN", dice_number, None, bot_number+1)
